...
if .rw_debug >= 2 
	...
	; send request to host to read disk image block 
	;  HL - block number to read 
	;  DE - address to put data read 
	;     - block size assumed to be 512-bytes 
	ld	a,host_read_block_cmd 
	out	(host_sd_cmd_port),a 
...
.bios_write_sdbuf: 
	...
	; send request to host to write disk image block 
	;  HL - block number 
	;  DE - address for data to write 
	;     - block size assumed to be 512-bytes 
	ld	a,host_write_block_cmd 
	out	(host_sd_cmd_port),a 
